...
const supHeroCard = document.querySelector('.suphero-card');
//const heroes = JSON.parse(supHeroCards);

    //Создаём переменные к каждому элементу
const createCard = (obj) => {
    const block = document.createElement("div");////Отображаем основной контейнер 
    block.classList.add("block");
    
    const name = document.createElement('span');//Отображаем блок Name
    name.className = "suphero-card__name";
    name.textContent = obj.name;
    
    const universe = document.createElement ('span');//Отображаем блок Universe
    universe.className = "suphero-card__universe";
    universe.textContent = "Вселенная: " + obj.universe;
    
    const alterego = document.createElement ('span');//Отображаем блок Alter Ego
    alterego.className = "suphero-card__alterego";
    alterego.textContent = "Альтер эго: " + obj.alterego;
    
    const occupation = document.createElement ('span');//Отображаем блок Occupation
    occupation.className = "suphero-card__occupation";
    occupation.textContent = "Род деятельности: " + obj.occupation;
    
    const friends = document.createElement ('span');//Отображаем блок Friends
    friends.className = "suphero-card__friends";
    friends.textContent = "Друзья: " + obj.friends;
    
    const superpowers = document.createElement ('span');//Отображаем блок Superpower
    superpowers.className = "suphero-card__superpowers";
    superpowers.textContent = "Супер-сила: " + obj.superpowers;

    const image = document.createElement("img");//Отображаем блок Images
    image.className = "suphero-card__img";
    image.src = obj.url;
    //console.log(obj);
    
    const info = document.createElement("span");//Отображаем блок Info
    info.className = "suphero-card__info";
    info.textContent = obj.info;

    
    //Отображаем блок Rating (5 Stars)
    ...
    input1.type = "radio";
    input1.className = "rating-input";
    input1.value = "1";
    input1.name = "rating";
    const span1 = document.createElement("span");
    span1.className = "rating-input__star";
    span1.id = "1";
    span1.textContent = "★";
    ...
    input2.type = "radio";
    input2.className = "rating-input";
    input2.value = "2";
    input2.name = "rating";
    const span2 = document.createElement("span");
    span2.className = "rating-input__star";
    span2.id = "2";
    span2.textContent = "★";
    ...
    input3.type = "radio";
    input3.className = "rating-input";
    input3.value = "3";
    input3.name = "rating";
    const span3 = document.createElement("span");
    span3.className = "rating-input__star";
    span3.id = "3";
    span3.textContent = "★";
    ...
    input5.type = "radio";
    input5.className = "rating-input";
    input5.value = "5";
    input5.name = "rating";
    const span5 = document.createElement("span");
    span5.className = "rating-input__star";
    span5.id = "5";
    span5.textContent = "★";

    if (localStorage.getItem(rating.id)) {
        const savedScore = localStorage.getItem(rating.id);
        //console.log(savedScore);
        const olgStarImputs = rating.querySelectorAll(".icon");
    
        //проверяем сколько звезд отмечено
        for (let i = 0; i < savedScore; i++) {
          //console.log(olgStarImputs[i]);
        olgStarImputs[i].style = "color: gold";
        }
    }
    //Расставляем все карточки по порядку
    block.append(name);
    block.append(universe);
    block.append(alterego);
    block.append(occupation);
    block.append(friends);
    block.append(superpowers);
    block.append(image);
    block.append(info);
    block.append(rating);

    //Расставляем все звезды 
    rating.append(label1);
    rating.append(label2);
    rating.append(label3);
    rating.append(label4);
    rating.append(label5);
...
document.addEventListener('DOMContentLoaded', () =>{
    supHeroCards.forEach((element) => {
        addCard(element, supHeroCard);
    }) 
});

//Функция для для закрашивания звезд
supHeroCard.addEventListener ("click", (e) => {
if (e.target.classList.contains("rating-input")){
    const form = e.target.parentNode.parentNode;//Переменная и поиск формы (также её дочерние элементы)
    const inputs = form.querySelectorAll(".rating-input");//Переменная для инпутов и их нахождение
    const icons = form.querySelectorAll(".rating-input__star");//Переменная для спанов и их нахождение
    const key = e.target.id;
    const value = Number(e.target.value);
    

////Функция для для закрашивания звезд в черный цвет
